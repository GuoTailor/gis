...
public class YearQuarterTableShardingPolicyImpl implements TableShardingPolicy {


    private static final Pattern tableNameYearPattern = Pattern.compile("^.+\\d{4}[qQ]\\d$");

    private static final Pattern tableNameYearAndIndexPattern = Pattern.compile("^(.+\\d{4}[qQ]\\d_)(\\d+)$");

    @Override
    public TableShardingPolicyTypeEnum forPolicy() {
        return TableShardingPolicyTypeEnum.YEAR_QUARTER;
    }

    @Override
    public String generateShardingTableName(String baseTableName, LocalDateTime time) {
        var year = time.getYear();
        var quarter = getQuarterFromTime(time);
        return baseTableName + year + "Q" + quarter;

    }

    private int getQuarterFromTime(LocalDateTime time) {
        //月份从1开始
