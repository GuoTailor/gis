...
public class YearTableShardingPolicyImpl implements TableShardingPolicy {


    private static final Pattern tableNameYearPattern = Pattern.compile("^.+\\d{4}$");

    private static final Pattern tableNameYearAndIndexPattern = Pattern.compile("^(.+\\d{4}_)(\\d+)$");

    @Override
    public String generateShardingTableName(String baseTableName, LocalDateTime time) {

        return baseTableName + time.getYear();
    }
    ...
    public TableShardingPolicyTypeEnum forPolicy() {
        return TableShardingPolicyTypeEnum.YEAR;
    }
}
